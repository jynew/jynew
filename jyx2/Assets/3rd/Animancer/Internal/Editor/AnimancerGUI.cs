...
namespace Animancer.Editor
{
    ...
    public static class AnimancerGUI
    {
        ...
        private static ConversionCache<float, string> _NormalizedSuffixCache, _SecondsSuffixCache;
        ...
        {
            if (_NormalizedSuffixCache == null)
            {
                _NormalizedSuffixCache = new ConversionCache<float, string>((x) => x + "x");
                _SecondsSuffixCache = new ConversionCache<float, string>((s) => s + "s");
            }
            ...
            if (showNormalized)
            {
                ...
                EditorGUI.BeginChangeCheck();
                normalizedTime = DoSpecialFloatField(normalizedArea, label, normalizedTime, _NormalizedSuffixCache);
                if (EditorGUI.EndChangeCheck())
            ...
            if (showSeconds)
            {
                ...
                EditorGUI.BeginChangeCheck();
                rawTime = DoSpecialFloatField(secondsArea, label, rawTime, _SecondsSuffixCache);
                if (EditorGUI.EndChangeCheck())
                {
                    if (timeIsNormalized)
                    {
