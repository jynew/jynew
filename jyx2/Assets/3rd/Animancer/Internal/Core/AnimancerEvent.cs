...
namespace Animancer
{
    ...
    public partial struct AnimancerEvent : IEquatable<AnimancerEvent>
    {
        ...
        public void AppendDetails(StringBuilder text, string name, string separator = "\n")
        {
            text.Append(separator).Append(name).Append(".NormalizedTime: ").Append(normalizedTime);

            if (callback != null)
            {
                text.Append(separator).Append(name).Append($".{nameof(callback.Target)}: ").Append(callback.Target);
                text.Append(separator).Append(name).Append($".{nameof(callback.Method)}: ").Append(callback.Method);
            }
        }
