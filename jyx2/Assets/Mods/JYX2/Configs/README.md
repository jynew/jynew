...
- `A2`单元格中填写这个配置表的名称，生成Lua配置表之后，可以在游戏中使用这个名称来访问该配置表，例如，当在Lua脚本中将配置表命名为`configMgr`时，用`configMgr.Character`指代名为`Character`的配置表。
...
### 字段名称
使用拉丁字符表示，生成Lua配置表后，可以在游戏中使用该名称访问这个字段。例如`configMgr.Character[0].Name`为配置表`Character`中第1条数据的`Name`字段的值。
### 数据
每一行为一条数据，`Id`字段的值可以用来访问这条数据，例如使用`configMgr.Character[233]`可以读取配置表`Character`中`Id`为`233`的那条数据。
...
## Lua配置表使用方法
生成的配置表会在游戏初始化时被读入`Jyx2ConfigMgr`这个Lua类中。在Lua脚本中使用如下代码可以访问Lua配置表，具体写法可以自由组合。
```Lua
-- 引入相关文件
require 'Jyx2Configs/Jyx2ConfigMgr'
-- 获取配置表单例
local configMgr = Jyx2ConfigMgr:Instance()
-- 获取名为 Character 的配置表，下面三种方式都可以
