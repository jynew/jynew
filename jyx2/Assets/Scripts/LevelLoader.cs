...
namespace Jyx2
{
    public class LevelLoader
    {
        ...
        static async UniTask DoLoad(Jyx2ConfigMap map, Action callback)
        {
            ...
            await LoadingPanel.Create($"{GameConfigDatabase.Instance.ModRootDir}/Maps/GameMaps/{map.MapScene}");
            callback?.Invoke();
        }
        ...
        private static async UniTask DoloadBattle(Jyx2ConfigBattle battle, Action<BattleResult> callback)
        {
            ...
            await LoadingPanel.Create($"{GameConfigDatabase.Instance.ModRootDir}/Maps/BattlesMaps/{battle.MapScene}");
