...
namespace Jyx2
{
    public class RoleInstance : SaveablePojo, IComparable<RoleInstance>
    {
        ...
        */
        ...
        #region Trigger
        private Dictionary<string, List<TriggerInstance>> _allTriggersMap;
        private Dictionary<string, TriggerInstance> _mergedTriggersMap;

        /// <summary>
        /// 获取指定pk的已合并TriggerInstance
        /// </summary>
        /// <returns></returns>
        public TriggerInstance GetTrigger(string pk)
        {
            if (_mergedTriggersMap == null || !_mergedTriggersMap.ContainsKey(pk)) return null;
            return _mergedTriggersMap[pk];
        }

        /// <summary>
        /// 获取所有已合并TriggerInstance
        /// </summary>
        /// <param name="pk"></param>
        /// <returns></returns>
        public List<TriggerInstance> GetAllTrigger()
        {
            if (_mergedTriggersMap == null || _mergedTriggersMap.Count == 0) return null;
            return _mergedTriggersMap.Values.ToList();
        }

        /// <summary>
        /// 获取指定key的已合并TriggerInstance列表
        /// </summary>
        /// <returns></returns>
        public IEnumerable<TriggerInstance> GetTriggers(string key)
        {
            if (_mergedTriggersMap == null) return null;
            return _mergedTriggersMap.Where(trigger => trigger.Key.Contains(key)).Select(trigger => trigger.Value);
        }

        /// <summary>
        /// 获取所有TriggerInstance列表（一般只用于回溯，禁止用于频繁操作的逻辑）
        /// </summary>
        /// <returns></returns>
        public List<TriggerInstance> GetFromAllTriggers(string pk)
        {
            if (_allTriggersMap == null || !_allTriggersMap.ContainsKey(pk)) return null;
            return _allTriggersMap[pk];
        }
        ...
        {
            if (_allTriggersMap == null) _allTriggersMap = new Dictionary<string, List<TriggerInstance>>();
            if (_mergedTriggersMap == null) _mergedTriggersMap = new Dictionary<string, TriggerInstance>();
            ...
            {
                _allTriggersMap.Add(trigger.PK, new List<TriggerInstance>() { trigger });
            }
        ...
        {
            if (_allTriggersMap == null) return;
            if (_mergedTriggersMap == null) return;
            ...
            if (_allTriggersMap.ContainsKey(trigger.PK))
            {
                ...
                if (_allTriggersMap[trigger.PK].Count == 0) _allTriggersMap.Remove(trigger.PK);
            }
            ...
            if (_mergedTriggersMap.ContainsKey(trigger.PK))
            {
                ...
                if (_mergedTriggersMap[trigger.PK].Count == 0) _mergedTriggersMap.Remove(trigger.PK);
            }
        }
        ...
        /// <param name="pk"></param>
        /// <returns></returns>
        public bool HasTrigger(string pk)
        {
            if (_allTriggersMap.ContainsKey(pk)) return true;
            return false;
        }
        ...
        {
            var trigger = GetTrigger($"Attribute:{attribute}");
            if (trigger == null) return 0;
            return Convert.ToInt32(trigger.KeyArgv);
        }
        #endregion
